...
if(isset($_POST['delete_category_id'])) {
    ...
    $delete_query = "DELETE FROM kategorier WHERE id = $delete_category_id";
...
if(isset($_POST['add_category'])) {
    ...
    $check_query = "SELECT COUNT(*) as count FROM kategorier WHERE name = '$new_category_name'";
    $check_result = $conn->query($check_query);
    $row = $check_result->fetch_assoc();
    $category_count = $row['count'];

    if($category_count > 0) {
        // Kategorien eksisterer allerede, vis alerten
        $alert_style = 'block';
        $alert_type = 'yellow';
        $alert_message = "Kategorien du prøvde å legge til eksisterer allerede.";
    } else {
        // Kategorien eksisterer ikke, legg til i databasen
    ...
    // Skjul alerten hvis ikke lagt til kategori
    $alert_style = 'hidden';
}
